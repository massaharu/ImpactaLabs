<?php
		...
		if($online == 0){
			...
			foreach($resultado as $e){
				...
				array_push($arrAvaliacao,array(
					...
					//"cadastrado"=>date('d-m-Y H:i',strtotime($e["DataCadastro"]))
					"cadastrado"=>date("d-m-Y", timestamp($e["DataCadastro"]))
				));
	...
	echo json_encode(array("myData"=>(count($arrAvaliacao)> 0) ? $arrAvaliacao : NULL))
