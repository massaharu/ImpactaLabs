...
<script type="text/javascript">
	...
	var store1 = new Ext.data.JsonStore({
		url:'/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/treinamentoslistados_list.php',	
		root:'myData',
	...
	var store = new Ext.data.JsonStore({
		url:'/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/gerenciadortreinamentos_list.php',	
		root:'myData',
		fields:[{name:'descurso',type:'string'}, 
										...
										handler:function(){
											if(Ext.getCmp('formaddtreinamento').getForm().isValid()){
												Ext.getCmp('formaddtreinamento').getForm().submit({
													url:'/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/treinamento_save.php',
													params:{
										...
										handler:function(btn){
											...
											if(Ext.getCmp('formaddrequisitosugestao').getForm().isValid()){
												...
												Ext.getCmp('formaddrequisitosugestao').getForm().submit({
													url:'/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/gerenciadortreinamentos_save.php',
						...
						Ext.MessageBox.erro('Aviso!', 'Por favor, você deve selecionar um registro para deletar.');
						 ...
						 Ext.MessageBox.confirm('Confirmação', 'Deletar o registro selecionado?',
							 function(btn){
								 if(btn=='yes'){
									Ext.Ajax.request({
									url: '/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/gerenciadortreinamentos_remove.php',
									params:{ 
			...
			items:[{				
				...
				listeners:{
				 afteredit: function(e){				
					  Ext.Ajax.request({ 
						  url: '/simpacweb/labs/Massaharu/extjsTelas/07.AdmSugestaoSite/gerenciadortreinamentos_update.php',
							  params:{
...
//////////////////////////////////////FILTRO/////////////////////////////////////////////////////////////////////////////////////				
				bbar:[{
					...
					listeners:{
						'valid':function(){
							if(Ext.getCmp('radiotreinamento').checked){
								store.filter('descurso', this.getValue(), true, false);
							}else if(Ext.getCmp('radiorequisito').checked){
								store.filter('descursoprerequisito', this.getValue(), true, false);							
							}else if(Ext.getCmp('radiosugestao').checked){
								store.filter('descursoavancado', this.getValue(), true, false);
							}
						}
					}
				}]
