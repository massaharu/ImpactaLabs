...
var storearquivoretorno_list = new Ext.data.JsonStore({
		url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/json/arquivoretorno_list.php',
		root:'myData',
	...
	var storearquivoretornotransacaobyarquivo_list = new Ext.data.JsonStore({
		url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/json/arquivoretornotransacao_list.php',
		root:'myData',
	...
	var storearquivoocorrencias_list = new Ext.data.JsonStore({
		url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/json/arquivoocorrencias_list.php',
		root:'myData',
						...
						bbar:['Cod. Ocorrência: ',{
							...
							handler:function(){
								...
								if(fn_validFields('nrcodocorrencia') && fn_validFields('txtocorrencia')){
									Ext.Ajax.request({ 
										url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/ajax/arquivoocorrencias_save.php',
										params:{
							...
							handler:function(){
								Ext.Ajax.request({ 
									url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/ajax/arquivoocorrencias_delete.php',
									params:{
						...
						tbar:['De: ',{
							...
							listeners:{
								'select':function(a){
									...
									if(fn_getDateInt($dateDe) > fn_getDateInt($dateAte)){
										xt('arquivoretorno_inicio').setValue($dateDe);
									}
		...
		'afterrender':function(){
			...
			var file = function(file){
				...
				var upload_file = function(File, new_name){
					...
					xhr.open('POST', '/simpacweb/modulos/financeiro/baixaBoletoAutomatico/ajax/arqRetornoBoletoItau_save.php', true);
							...
							if(result.success){
								...
								$('#file_to_input').next().fadeOut(2000, function(){
									Ext.MessageBox.info("Arquivo salvo e processado com sucesso!"," Clique em OK para visualizar os pagamentos baixados/não baixados.", function(){
										...
										Ext.Ajax.request({
											url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/ajax/pagamentoBaixaBoleto_load.php',
											params:{
											...
											success:function(response){
												...
												$pagtonaobaixado_dac = ($pagtonaobaixado_dac)? $pagtonaobaixado_dac : "N/A"
												$.each($myJson.pagtoNaoEncontrado, function(){
													$pagtonaoencontrado_dac+= this.num_documento+' - '+this.nome_sacado+', ';
												});
												$pagtonaoencontrado_dac = ($pagtonaoencontrado_dac)? $pagtonaoencontrado_dac : "N/A";
												...
												$.each($myJson.myData, function(){
													...
													if(this.matricula){
														...
														var $LinkPermissaoMatricula = "N/A";
													}
													...
													$content_data+= (
														'<tr data-recibostatus="'+$recibostatus+'" data-sessionid="'+this.sessionid+'">'+
															...
															'<td class="nome_cli">'+$LinkPermissaoAccount+'</td>'+
															...
															'<td class="dtconfirmpagto" data-dtcadastro="'+this.dtocorrencia+'">'+
													...
													/*$('#myTable tr[data-sessionid='+this.sessionid+'] td.dtconfirmpagto .myDatepicker').val(fn_getDateStringFormated(new Date(1000 * this.dtcadastro)));
													$('#myTable tr[data-sessionid='+this.sessionid+'] td.dtconfirmpagto .myTimefield').val(fn_getTimeStringFormated(new Date(1000 * this.dtcadastro)));*/
												});
												...
												$content = (
													'<table id="myTable">'+
														'<tr>'+
															...
															'<th width="15%">Forma de Pagto.</th>'+
															'<th width="15%">Matrícula</th>'+
															'<th width="25%">Data de Pagto</th>'+
														'</tr>'
														+$content_data+	
														'<tr>'+
												...
												if(xt('idwinPagamentoNaoBaixado')){
													...
													var winPagamentoNaoBaixado = new Ext.Window({
														...
														html:$content,
														buttons:[{
															text:'<b>Confirmar Pagamento</b>',
															id:'btnconfirmpagto',
															iconCls:'ico_accept',
															scale:'large',
															width:200
														},{
															text:'<b>Imprimir Lista</b>',
															id:'btnimprimirlista',
															iconCls:'ico_print',
															scale:'large',
															width:200
															...
															'afterrender':function(){
																$(function() {
																	...
																	$.each($baixaDados, function(){
																		var $dtpagtovalue = $(this).find('td.dtconfirmpagto').attr('data-dtcadastro');
																		var $sessionid = $(this).attr('data-sessionid');
																		
																		$('#myTable tr[data-sessionid='+$sessionid+'] td.dtconfirmpagto .myDatepicker').val(fn_getDateStringFormated(new Date(1000 * $dtpagtovalue)));
																		$('#myTable tr[data-sessionid='+$sessionid+'] td.dtconfirmpagto .myTimefield').val(fn_getTimeStringFormated(new Date(1000 * $dtpagtovalue)));
																	});
																	...
																	$('#btnconfirmpagto').on('click', function(){
																		...
																		$.each($objBaixaDados, function(){
																			if(this.sessionid=="false") $hasSessionid = false;
																			if(this.recibostatus=="false") $isAllPago = false;
																		});
																		
																		if($hasSessionid){
																			if(!$isAllPago){
																				Ext.Ajax.request({
																					url:'/simpacweb/modulos/financeiro/baixaBoletoAutomatico/ajax/pagamentoBaixaBoleto_save.php',
																					params: {
																	...
																	//EVENTO CLICK DO BOTAO IMPRIMIR LISTA
																	$('#btnimprimirlista').on('click', function(){
																		window.open('/simpacweb/modulos/financeiro/baixaBoletoAutomatico/baixaBoletoAutomatico_print.php?idarquivo='+result.archive_details[0].idarquivo,'Print','width=1000, height=700, top=0, left=0, scrollbars=yes, status=no, toolbar=no, location=no, directories=no, addressbar=no, menubar=no, resizable=yes,maximize=yes');
																	});
																});
															}
														}
