...
SELECT * FROM tb_oportunidades where idoportunidade = 423
SELECT * FROM tb_oportunidadesfit where idoportunidade = 427
...
SELECT * FROM SOPHIA.SOPHIA.FISICA where NOME like '%maruf%'
SELECT * FROM tb_pessoassophia
SELECT * FROM tb_pessoas where idpessoa = 1027241

sp_oportunidadepagamentosbyidoportunidadepai_get 354

SELECT * FROM tb_pagamentos where idpagamento = 10012

SELECt * FROM tb_oportunidadesfitdescontos where idoportunidadefit = 124
SELECT * FROM tb_descontosbolsasfit
SELECT * FROM tb_oportunidadeetapatipos
SELECT * FROM tb_departamentos
SELECT * FROM tb_descontos WHERE iddesconto = 23

exec sonata.sophia.sophia.sp_uf_list

sp_oportunidadesfit_get 1

SELECT * FROM SONATA.SOPHIA.SOPHIA.DADOSPF
SELECT * FROM SONATA.SOPHIA.SOPHIA.CIDADES

SELECT * FROM tb_oportunidadesfit WHERE 
...
SELECT * FROM tb_pessoassophia
SELECT * FROM SOPHIA.SOPHIA.FISICA WHERE CODIGO = 1252
SELECT * FROM SOPHIA.SOPHIA.FISICA WHERE NOME LIKE '%Lindsay Lohan%'
SELECT * FROM tb_pessoas WHERE despessoa LIKE '%Lindsay Lohan%'
SELECT * FROM tb_documentos where idpessoa = 1027241
SELECt * FROM tb_contatos where idpessoa = 1027241
...
sp_oportunidadesfitsophia_save 124, 2359, 1
--  
select 
      c.CODIGO,
      c.NOME
...
CREATE TABLE tb_oportunidadesfitsophia(
	idoportunidadesfitsophia int IDENTITY CONSTRAINT PK_oportunidadesfitsophia PRIMARY KEY ,
...
DROP TABLE tb_oportunidadesfitsophia(
	idoportunidadesfitsophia int IDENTITY CONSTRAINT PK_oportunidades2fitsophia PRIMARY KEY ,
	idoportunidadefit int,
	idturma int not null,
	nrordem smallint
	
	CONSTRAINT FK_oportunidades2fitsophia_oportunidadefit_idoportunidadefit FOREIGN KEY(idoportunidadefit)
	REFERENCES tb_oportunidadesfit (idoportunidadefit)
)
...
sp_oportunidadealuno_list '72469606780', 404
sp_oportunidadealuno_list NULL, 404
--
sp_oportunidadealuno_list null, 427
sp_oportunidadealuno_list 34784785884, NULL
sp_oportunidadealuno_list NULL, NULL
...
SELECT * FROM tb_pessoassophia
SELECT * FROM tb_pessoas where idpessoa = 1021843
SELECt * FROM SOPHIA.SOPHIA.FISICA where CODIGO IN (26343,26357)
SELECT * FROM tb_contatos
SELECT * FROM tb_contatotipos
SELECT * FROM tb_documentos where desdocumento like '%.%'
...
END
--------------------------------------------------------------------------------
CREATE PROC sp_oportunidadealunobydespessoa_list
(
...
BEGIN
	...
	LEFT JOIN tb_pessoassophia o ON o.idpessoa = a.idpessoa
	LEFT JOIN SOPHIA.SOPHIA.FISICA p ON p.CODIGO = o.codigo     
	WHERE a.despessoa like '%'+@despessoa+'%'
	ORDER BY e.dtcadastro DESC            
END
...
BEGIN
	...
	INNER JOIN SOPHIA.SOPHIA.CONCURSOS b ON b.CODIGO = a.idvestibular
	LEFT JOIN fit_new.dbo.cadcadastro c ON c.cadastroId = a.idpalestra
...
BEGIN
	SELECT
		a.idoportunidadefit, a.idoportunidade, b.idprodutotipo, c.desprodutotipo, a.idproduto, 
		b.desproduto, a.intransferido, a.infies, a.inprouni, a.dtcadastro, a.instatus, 
		a.nrparcelas as nrparcelasoportunidadefit, a.dtvencimentomatricula, a.inboletomatriculagerado, 
		a.indocumentoliberado, a.inboletovestibulargerado, d.idpessoa, a.inenem, a.nrnotaenem
...
SELECT * FROM tb_pessoassophia
SELECT * FROM tb_pessoas
SELECT * FROM SOPHIA.SOPHIA.FISICA where CODIGO = 26332
...
--------------------------------------------------------------------------------
ALTER PROC [dbo].[sp_descontositensbydesconto_list]
...
BEGIN
	...
	select MAX(codigo)as codigo, curso from SOPHIA.SOPHIA.CURRICULO
	group by curso
	order by curso
	...
	inner join SOPHIA.SOPHIA.SERIE c on c.curriculo = b.codigo
	where a.idproduto = @idproduto
END
--------------------------------------------------------------------------------
ALTER PROC sp_oportunidadesfitsophia_get --56
(@idoportunidadefit int)
AS
/*
  app:CRM
...
BEGIN
      ...
      INNER JOIN SOPHIA.SOPHIA.TURMAS b ON b.CODIGO = a.idturma
      INNER JOIN SOPHIA.SOPHIA.PERIODOS c ON c.CODIGO = b.PERIODO
      WHERE idoportunidadefit = @idoportunidadefit
END
...
BEGIN
	...
	FROM SOPHIA.SOPHIA.PRODUTOS A
	INNER JOIN SOPHIA.SOPHIA.PLANOCONTAS B ON A.CLASSIFIC_MATR = B.CODIGO 
	INNER JOIN SOPHIA.SOPHIA.PLANOCONTAS C ON A.CLASSIFICACAO = C.CODIGO
	WHERE A.DESCRICAO IN(SELECT NOME_RESUM FROM SOPHIA.SOPHIA.CURSOS WHERE PRODUTO = @idcurso)
END	
--------------------------------------------------------------------------------
...
BEGIN
	...
	INNER JOIN SOPHIA.SOPHIA.FISICA c ON c.CODIGO = b.codigo
	WHERE a.idpessoa = @idpessoa
END


SELECT * FROM SOPHIA.SOPHIA.FISICA WHERE CODIGO = 26283
SELECT * FROM tb_pessoassophia 
--------------------------------------------------------------------------------
...
BEGIN
	...
	INNER JOIN SOPHIA.SOPHIA.CURSOS c ON c.PRODUTO = b.produto
	WHERE a.idproduto = @idproduto
END
--
SELECT * FROM tb_produtos_sophiacursos where produto = 57
SELECT * FROM SOPHIA.SOPHIA.CURSOS
...
BEGIN
	SELECT top 1 
		a.idoportunidade, a.idpessoa, a.inuniversidade, a.idendereco, a.instatus, a.desemail, 
...
BEGIN
	IF @codigo IS NULL
		SELECT CODIGO, DESCRICAO FROM SOPHIA.SOPHIA.DOCUMENTOS
		ORDER BY DESCRICAO
	ELSE
		SELECT CODIGO, DESCRICAO FROM SOPHIA.SOPHIA.DOCUMENTOS
		WHERE CODIGO = @codigo
...
END
--------------------------------------------------------------------------------
ALTER PROC sp_oportunidadesfitdocs_save
(
	@idoportunidadefit int,
...
BEGIN
	...
	INSERT INTO tb_oportunidadesfitdocs (idoportunidadefit, documento, inapresentado)
	SELECT @idoportunidadefit, id, @inapresentado FROM Simpac.dbo.fnSplit(@documentos, ',')
END
--
SELECt * FROM tb_oportunidadesfitdocs
SELECT * FROM SONATA.SOPHIA.SOPHIA.DOCUMENTOS
--------------------------------------------------------------------------------
CREATE PROC sp_oportunidadesfitdocs_list
(@idoportunidadefit int)
AS
BEGIN
	SELECT
...
--------------------------------------------------------------------------------
ALTER PROC sp_oportunidadesfitarquivos_list
...
select * from tb_pessoas where idpessoa = 427

SELECT * FROM tb_oportunidadesfit a
INNER JOIN tb_oportunidades b ON b.idoportunidade = a.idoportunidade
INNER JOIN tb_oportunidadearquivos c ON c.idoportunidade = b.idoportunidade
INNER JOIN tb_arquivos d ON d.idarquivo = c.idarquivo
WHERE b.idoportunidade = 427
--------------------------------------------------------------------------------
...
SELECT * FROM SOPHIA.SOPHIA.FISICA WHERE NOME LIKE '%Julio Maluf Crm%'
